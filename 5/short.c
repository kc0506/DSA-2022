...
ll heap[MAX_A * MAX_N], index_table[MAX_A * MAX_N], result[MAX_K];
...
ll RecurSub6(int s, int k){
    ...
    if(ReLPR + StLPR >= k){
        ...
        if(ReMidVal < StMidVal){
            ...
            StStart = MAX(StMid - N + 1, StStart), k -= StStart - CurStart;
        }
    }
    else{
        // printf("case3\n");
